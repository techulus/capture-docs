...
| stealth               | false             | Use faster stealth mode to avoid triggering bot challenges                                 |
...
## Stealth vs. Bot Detection Bypass

Use `stealth=true` when you want a faster capture mode that reduces the chance of triggering bot challenges. Stealth mode cannot solve captchas; it tries to avoid causing them.

Use `bypassBotDetection=true` when a page is expected to show bot detection or captchas. This mode can solve captchas, but it is slower than stealth mode.
